...
class TwitEng
{
 public:
  ...
  std::map<std::string,std::set<Tweet*>> tags_set() const; 
  ...
  std::map<std::string, User*> usersSet() const;

  /* You may add other member functions */
 private:
  /* Add any other data members or helper functions here  */
    std::map<std::string, User*> twi_users;
    std::map<std::string, std::set<Tweet*>> tags;
    std::set<Tweet*> tew;
};
